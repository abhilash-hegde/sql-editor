...
export default {
  ...
  getRecentOrders: (orders) => {
    return orders.sort(byDate).slice(0, 50);
  },
  getAllOrderDetails: (
    orders,
    customers,
    ...
    return orders.map((order) => {
      ...
      const employee =
        employees.find((e) => e.employeeID === order.employeeID) || {};
      ...
      const orderDetails = order.details.map((detail) => {
        ...
        const supplier =
          suppliers.find((sup) => sup.supplierID === product.supplierID) || {};
  ...
  getTotalSalesPerCustomer: (orders, customers) => {
    return orders
      .map((order) => {
        const customer =
          customers.find((c) => c.customerID === order.customerID) || {};
        const totalSpent = order.details.reduce(
    ...
    orders.forEach((order) => {
      order.details.forEach((detail) => {
        productSales[detail.productID] =
          (productSales[detail.productID] || 0) + detail.quantity;
      });
    });
    ...
    orders.forEach((order) => {
      order.details.forEach((detail) => {
        ...
        categorySales[categoryName] =
          ...
          detail.unitPrice * detail.quantity * (1 - detail.discount);
      });
    });
  ...
  getEmployeesWithMostOrders: (orders, employees) => {
    ...
    orders.forEach((order) => {
      employeeOrders[order.employeeID] =
        (employeeOrders[order.employeeID] || 0) + 1;
    });
  ...
  getLateShippedOrders: (orders, customers) => {
    return orders
      .filter(
        (order) => new Date(order.shippedDate) > new Date(order.requiredDate)
      )
      .map((order) => {
        const customer =
          customers.find((c) => c.customerID === order.customerID) || {};
        ...
        return {
          ...
          shippedDate: order.shippedDate,
          daysLate:
            (new Date(order.shippedDate) - new Date(order.requiredDate)) /
            millisInADay,
        };
      })
    ...
    orders.forEach((order) => {
      order.details.forEach((detail) => {
        const product =
          products.find((p) => p.productID === detail.productID) || {};
        const supplier =
          suppliers.find((s) => s.supplierID === product.supplierID) || {};
        const supplierName = supplier.companyName || "Unknown";
        supplierRevenue[supplierName] =
          (supplierRevenue[supplierName] || 0) +
          detail.unitPrice * detail.quantity * (1 - detail.discount);
      });
    });
        ...
        order.details
          ...
          .map((detail) => {
            ...
            return {
              ...
              discount: detail.discount,
              discountAmount:
                detail.unitPrice * detail.quantity * detail.discount,
            };
          })
      )
