            ...
            <TableBody>
                {schema.map((column) => (
                    <Fragment key={column.columnName}>
                        <TableRow className={level === 0 ? "bg-gray-100 font-bold" : "pl-4"}>
                            <TableCell className={level === 0 ? "font-semibold" : "pl-6"}>{column.columnName}</TableCell>
                            <TableCell>{column.dataType}</TableCell>
                        </TableRow>
                        {column.children && (
                            <TableRow>
                                <TableCell colSpan={2} className="pl-8">
                                    <TableSchemaContent schema={column.children} level={level + 1} />
                                </TableCell>
                            </TableRow>
                        )}
                    </Fragment>
