    ...
    >
      <CollapsibleTrigger asChild>
        <Button variant="text" className="w-full justify-between">
          {table}
          {openTable === table ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
        </Button>
      </CollapsibleTrigger>
      <CollapsibleContent>
        <TableActions table={table} schemaTable={schemaTable} setSchemaTable={setSchemaTable} onViewData={onViewData} />
      </CollapsibleContent>
    </Collapsible>
  );
  ...
  return (
    <>
      <div className="flex space-x-2 align-center mt-1 pb-1">
        <Button onClick={() => onViewData(table)} variant="outline" className="text-xs bg-white">
          <Table className="h-2 w-2 mr-1" />
          View Data
        </Button>
        <Button onClick={onSchemaVisibilityChange} variant="outline" className="text-xs bg-white">
          {schemaTable ? <EyeOff className="h-2 w-2 mr-1" /> : <Eye className="h-2 w-2 mr-1" />}
